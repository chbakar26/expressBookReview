...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (books[isbn]) {
      ...
      if (book.reviews[username]) {
          ...
          return res.status(200).send("Review for the ISBN " + isbn + " posted by the user " + username + " deleted successfully.");
      } else {
          return res.status(404).json({message: "Review by this user for the given ISBN not found"});
      }
  } else {
      return res.status(404).json({message: "ISBN not found"});
  }
